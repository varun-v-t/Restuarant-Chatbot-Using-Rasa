	...
	def run(self, dispatcher, tracker, domain):
		...
		if checkop is not None and checkop==True:
			...
			try:
				...
				results=zomato.restaurant_search_new("", lat, lon, str(cuisines_dict.get(cuisine.strip().lower())), 10000)
				d = json.loads(results)
				df = self.filter_by_budget(d,price)
				response="Showing you the top rated restaurants according to your choice: "
				...
				if df.size==0:
					...
					return [SlotSet('found_resto', False)]
				else:
				...
				return [SlotSet('location',loc),SlotSet('email_response',email_response),SlotSet('found_resto', True)]
			except Exception as e:
				import traceback
				traceback.print_exc()
			...
			return [SlotSet('check_op', False),SlotSet('found_resto', False)]
...
class CheckLocation(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		if validate_location(loc):
			print("location validity is True")
			return [SlotSet('check_op',True)]
		else:
			print("location validity is False")
...
class SendEmail(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		dispatcher.utter_message("Mail sent to your email id!")
		return []